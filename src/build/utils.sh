...
get_patches_key() {
	...
	# If no per-app patch lists exist, do nothing and let CLI defaults apply.
	if [ ! -f "src/patches/$1/include-patches" ] || [ ! -f "src/patches/$1/exclude-patches" ]; then
...
patch() {
	...
	if [ -f "./download/$1.apk" ]; then
		...
		if [ "$3" = inotia ]; then
			...
			# Use Morphe CLI with patches bundle; let CLI handle signing with its default keystore.
